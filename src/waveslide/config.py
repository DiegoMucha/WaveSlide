...
DEFAULT_LABELS = ("call", "fist", "like", "two_up", "unknown")


@dataclass(frozen=True)
class EngineConfig:
    model_path: Path = Path("models/WaveSlideV1.tflite")
    ...
    min_prediction_confidence: float = 0.90
...
def config_from_env() -> EngineConfig:
    return EngineConfig(
        model_path=Path(os.getenv("WAVESLIDE_MODEL_PATH", "models/WaveSlideV1.tflite")),
        hand_landmarker_path=Path(
            os.getenv("WAVESLIDE_HAND_LANDMARKER_PATH", "models/hand_landmarker.task")
        ),
        ...
        in {"1", "true", "yes"},
        min_prediction_confidence=float(os.getenv("WAVESLIDE_CONFIDENCE", "0.90")),
        prediction_interval_ms=int(os.getenv("WAVESLIDE_PREDICTION_INTERVAL_MS", "100")),
